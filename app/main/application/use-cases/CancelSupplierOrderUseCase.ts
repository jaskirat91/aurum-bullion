...
export class CancelSupplierOrderUseCase {
  async execute(voucherId: string): Promise<{ success: boolean }> {
    return AppDataSource.transaction(async (em) => {
      ...
      // 2. Find supplier order voucher
      const orderVoucherRepo = em.getRepository(SupplierOrderVoucher);
      const orderVoucher = await orderVoucherRepo.findOne({ where: { voucherId } });
      if (!orderVoucher) throw new Error('Supplier order voucher not found.');
      ...
      if (supplierAccount) {
        Account.validateFreezeDate(supplierAccount, voucher.entryDate);
      }
