...
export function GoldVoucherManager() {
  ...
  const handleDelete = async (voucherId: string) => {
    const confirmed = await confirm({
      ...
      message:
        'Are you sure you want to permanently delete this gold voucher? This will also remove all associated ledger entries.',
      type: 'warning',
    });
          ...
          <table className="w-full text-left border-collapse text-xs">
            <thead className="sticky top-0 bg-surface z-10 border-b border-border shadow-sm">
              <tr className="bg-background/40">
                ...
                <th className="border px-3 py-2 font-black uppercase tracking-widest text-right w-[110px]">
                  Cash AMT
                </th>
                <th className="border px-3 py-2 font-black uppercase tracking-widest text-right w-[100px]">
                  Gold WT
                </th>
                  ...
                  return (
                    <tr key={item.voucherId} className="group hover:bg-primary/5 transition-colors">
                      ...
                      <td className="border px-3 py-2 font-mono font-black">
                        {item.voucher?.voucherNo}
                      </td>
                      <td className="border px-3 py-2">
                        <div
                          className={`flex items-center gap-1 font-bold uppercase ${!isIssue ? 'text-emerald-500' : 'text-primary'}`}
                        >
                      ...
                      <td className="border px-3 py-2">
                        <div className="flex items-center gap-1.5 font-bold text-text/70 text-[10px]">
                          ...
                          {item.voucher?.entryDate
                            ? new Date(item.voucher.entryDate).toLocaleDateString()
                            : '—'}
                        </div>
                      </td>
                      <td className="border px-3 py-2">
                        <div className="font-bold text-text leading-none">
                          {item.partyAccount?.name ?? '—'}
                        </div>
                        <div className="text-[9px] font-black text-text-muted uppercase">
                          {item.partyAccount?.code}
                        </div>
                      </td>
                      <td className="border px-3 py-2">
                        {item.customerOrderVoucher ? (
                          <div className="font-bold text-text leading-none">
                            {item.customerOrderVoucher?.voucher?.voucherNo}
                          </div>
                        ) : (
                          <div className="text-[9px] font-black text-text-muted uppercase">
                            {item.supplierOrderVoucher?.voucher?.voucherNo}
                          </div>
                        )}
                      </td>
                      <td className="border px-3 py-2">
                        <p className="font-medium line-clamp-2" title={item.voucher?.narration}>
                          {item.voucher?.narration || (
                            <span className="text-text-muted/40 italic">No narration</span>
                          )}
                        </p>
                      </td>
                      <td className="border px-3 py-2">
                        <StatusChip status={item.voucher.status} />
                      </td>
                      <td
                        className={`border px-3 py-2 font-black text-right ${!isIssue ? 'text-emerald-600' : 'text-primary'}`}
                      >
