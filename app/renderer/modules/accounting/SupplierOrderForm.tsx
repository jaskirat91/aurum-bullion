...
export function SupplierOrderForm({ onCancel, onSuccess, editVoucherId, initialData }: SupplierOrderFormProps) {
  ...
  const handleSubmit = async (action: 'POST') => {
    ...
    try {
      ...
      const res = isEdit
        ? await window.electronAPI.updateSupplierOrder(editVoucherId!, dto)
        : await window.electronAPI.createSupplierOrder(dto);
      
      if (res.success) {
        const voucherNo = (res.data as any)?.voucherNo ?? '';
        const msg = action === 'POST'
          ? `Supplier Order posted successfully! Order No: ${voucherNo}`
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-black tracking-tight text-text leading-none">
                {isEdit ? 'Edit Supplier Order (Draft)' : 'Create Supplier Order'}
              </h2>
              <p className="text-[9px] font-black uppercase text-text-muted tracking-widest leading-none mt-0.5">
                {isEdit ? 'Update draft and optionally post' : 'Record a new supplier order'}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar px-6 py-5 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-6">
            ...
            <div className="space-y-5">
              ...
              <div>
                <label className="text-[9px] font-black uppercase tracking-widest text-text-muted block mb-1.5 flex items-center gap-1">
                  <UserCheck size={10} /> Supplier Account <span className="text-danger">*</span>
                </label>
                <AccountSelect
                  inputRef={partyRef}
                  label=""
