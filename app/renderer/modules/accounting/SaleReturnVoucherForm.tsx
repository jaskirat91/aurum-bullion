...
import { OldItemDialog } from './OldItemDialog';
import {
  Receipt, Calendar, FileText, UserCheck, Plus, Trash2, Pencil,
  RotateCcw, Save, SendHorizonal, Tag, Weight, IndianRupee,
  Percent, ArrowLeft, AlertCircle, History,
} from 'lucide-react';

interface SaleReturnVoucherFormProps {
  onCancel: () => void;
  onSuccess: () => void;
...
export function SaleReturnVoucherForm({ onCancel, onSuccess, editVoucherId, initialData }: SaleReturnVoucherFormProps) {
  ...
  const [showOldItemDialog, setShowOldItemDialog] = useState(false);
  ...
  // For system items — prevent duplicate productIds
  const handleAddItem = (item: LineItem) => {
    if (!item.isOldItem) {
      const duplicate = lineItems.find(li => !li.isOldItem && li.productId === item.productId);
      if (duplicate) {
  ...
  const handleEditItemClick = (item: LineItem, index: number) => {
    ...
    // Dual guard: isOldItem flag OR empty productId both indicate an old item
    const isOld = !!item.isOldItem || item.productId === '';
    if (isOld) {
      setShowLineItemDialog(false); // ensure system dialog is closed
      setShowOldItemDialog(true);
    } else {
      setShowOldItemDialog(false); // ensure old dialog is closed
    ...
    try {
      const dto = {
        ...
        items: lineItems.map(li => ({
          productId: li.isOldItem ? undefined : li.productId,
          isOldItem: li.isOldItem ?? false,
          oldItemName: li.oldItemName,
          oldItemTag: li.oldItemTag,
          tagGrossWeight: li.tagGrossWeight,
          tagKundanWeight: li.tagKundanWeight,
          tagStoneWeight: li.tagStoneWeight,
          tagMottiWeight: li.tagMottiWeight,
          tagNetWeight: li.tagNetWeight,
          tagAmount: li.tagAmount,
      ...
      const res = isEdit
        ? await window.electronAPI.updateSaleReturnVoucher(editVoucherId!, dto)
        : await window.electronAPI.createSaleReturnVoucher(dto);
      if (res.success) {
        const voucherNo = (res.data as any)?.voucherNo ?? '';
        const msg = action === 'POST'
          ? `Sale Return Voucher posted successfully! Voucher No: ${voucherNo}`
  ...
  return (
    <div className="h-full w-full flex overflow-hidden bg-background">
      ...
      <div className="flex-1 flex flex-col overflow-hidden border-r border-border/50">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-black tracking-tight text-text leading-none">
                {isEdit ? 'Edit Sale Return Voucher' : 'Create Sale Return Voucher'}
              </h2>
              <p className="text-[9px] font-black uppercase text-text-muted tracking-widest leading-none mt-0.5">
                {isEdit ? 'Update voucher details and optionally re-post' : 'Return of sold finished products from customers'}
              </p>
            </div>
          </div>
          {/* Totals summary */}
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar px-6 py-5 space-y-5">
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div className="col-span-2">
              ...
              <input
                ...
                placeholder="Optional note for this sale return voucher..."
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="flex items-center gap-2 px-3 py-2 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-600 text-[10px] font-black uppercase hover:bg-amber-500 hover:text-white hover:border-amber-500 transition-all shadow-sm disabled:opacity-40"
                  title="Add an item that was not sold through this system"
                >
                  <History size={13} /> Add Old Item
                </button>
                <button
                  type="button"
                  disabled={!customerId}
                  ...
                  className="flex items-center gap-2 px-4 py-2 rounded-xl bg-primary/10 border border-primary/20 text-primary text-[10px] font-black uppercase hover:bg-primary hover:text-white hover:border-primary transition-all shadow-sm disabled:opacity-40"
                >
            ...
            {!customerId ? (
              <div className="flex flex-col items-center justify-center gap-2 h-32 border-2 border-dashed border-border rounded-2xl bg-surface/10">
                <UserCheck size={24} className="text-text-muted/20" />
                <p className="text-[10px] font-black text-text-muted uppercase tracking-widest text-center px-6">
                  Please select a customer first to return their sold items
                </p>
              </div>
            ) : lineItems.length === 0 ? (
              <div className="flex flex-col items-center justify-center gap-3 h-32 border-2 border-dashed border-border rounded-2xl bg-surface/10">
                ...
                <p className="text-[10px] font-black text-text-muted uppercase tracking-widest text-center">
                  Use <span className="text-primary">Add Item</span> for system products or <span className="text-amber-500">Add Old Item</span> for items sold before this platform
                </p>
              </div>
            ) : (
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-1.5">
                            <span className="font-mono text-[10px] font-black bg-primary/10 text-primary px-2 py-0.5 rounded-lg">
                              {item.isOldItem ? (item.oldItemTag?.split('|')[0]?.trim() ?? 'OLD') : item.tag}
                            </span>
                            {item.isOldItem && (
                              <span className="text-[8px] font-black bg-amber-500/10 text-amber-600 px-1.5 py-0.5 rounded-md uppercase">Old</span>
                            )}
                          </div>
                        </td>
      ...
      {showOldItemDialog && (
        <OldItemDialog
          isOpen={showOldItemDialog}
          onClose={() => {
            setShowOldItemDialog(false);
            setEditingItem(null);
            setEditingIndex(null);
          }}
          onSave={editingItem ? handleUpdateItem : handleAddItem}
