...
export function SaleReturnVoucherDetailsOverlay({ voucherId, onClose, onEdit }: DetailsProps) {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      try {
        const res = await window.electronAPI.getSaleReturnVoucherDetails(voucherId);
        ...
        <div className="shrink-0 px-6 py-4 border-b border-border bg-background/30 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[9px] font-black uppercase text-text-muted tracking-widest leading-none mt-0.5">Sale Return Voucher Details</p>
            </div>
          </div>
              ...
              {details.items?.length > 0 && (
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="bg-emerald-500/5 border border-emerald-500/15 rounded-2xl p-4">
                    ...
                    <div className="text-xl font-black text-emerald-500 mt-0.5">₹ {totalAmount.toLocaleString()}</div>
                  </div>
                </div>
              )}
                    ...
                    <tbody className="divide-y divide-border/20">
                      {(details.items ?? []).map((item: any, idx: number) => (
                        <tr key={item.id ?? idx} className="hover:bg-primary/5 transition-colors">
                          ...
                          <td className="px-4 py-3 font-black text-emerald-500 text-right">₹ {item.labourAmount.toLocaleString()}</td>
                        </tr>
                      ))}
                    </tbody>
                    {details.items?.length > 0 && (
                      <tfoot>
                        <tr className="bg-background/40 border-t border-border/60">
                          ...
                          <td />
                          <td className="px-4 py-2.5 text-[12px] font-black text-emerald-500 text-right">₹ {totalAmount.toLocaleString()}</td>
                        </tr>
                      </tfoot>
                    )}
                  </table>
